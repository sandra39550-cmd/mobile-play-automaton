...
import { usePerception, PerceptionResult } from "@/hooks/usePerception";
...
export const PerceptionView = ({ deviceId, gameName }: PerceptionViewProps) => {
  const { latestPerception, perceptionHistory, isPerceiving, error, perceive, clearHistory } = usePerception();
  const [showHistory, setShowHistory] = useState(false);

  const handlePerceive = async () => {
    if (!deviceId || !gameName) return;
    await perceive(deviceId, gameName);
  };

  const renderPerceptionCard = (p: PerceptionResult, isLatest = false) => (
    <Card
      key={p.timestamp}
      className={`p-4 border-gaming-border bg-gaming-card ${isLatest ? "ring-1 ring-neon-purple/40" : "opacity-80"}`}
    >
      {/* Scene Understanding */}
      ...
      <div className="flex items-center gap-2 mb-3">
        ...
        <span className="text-xs font-mono text-neon-green">
          {(p.sceneUnderstanding.confidence * 100).toFixed(0)}%
        </span>
      </div>
        ...
        <div className="mb-3">
          <div className="flex items-center gap-1.5 mb-2">
            ...
            <span className="text-xs font-semibold">
              Detected Elements ({p.detectedElements.length})
            </span>
          </div>
          <div className="flex flex-wrap gap-1.5">
            {p.detectedElements.map((el, i) => (
              <Badge
                key={i}
                variant="outline"
                className={`text-xs ${el.actionable ? "border-neon-green/50 text-neon-green" : "border-muted text-muted-foreground"}`}
              >
                {el.label}
                <span className="ml-1 opacity-60">{(el.confidence * 100).toFixed(0)}%</span>
              </Badge>
            ))}
          </div>
        </div>
        ...
        <div className="mb-3">
          ...
          <div className="flex flex-wrap gap-1 mt-1">
            {p.screenText.map((t, i) => (
              <span key={i} className="text-xs bg-muted/50 px-1.5 py-0.5 rounded font-mono">
                {t}
              </span>
            ))}
          </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs font-mono mt-0.5">
                {p.suggestedAction.type} → ({p.suggestedAction.coordinates.x}, {p.suggestedAction.coordinates.y})
              </p>
            </div>
            <Badge variant="outline" className="text-neon-pink border-neon-pink text-xs">
              {(p.suggestedAction.confidence * 100).toFixed(0)}%
            </Badge>
          </div>
        </div>
      )}
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              ...
              {perceptionHistory.length}
            </Button>
            <Button
              size="sm"
              onClick={handlePerceive}
              disabled={isPerceiving || !deviceId}
        ...
        {!latestPerception && !isPerceiving && (
          <div className="text-center py-8 text-muted-foreground">
            ...
            <p className="text-xs mt-1">
              {deviceId ? "Click 'Perceive Now' to see what the agent sees" : "Connect a device and start a game first"}
            </p>
          </div>
        )}
              ...
              <div>
                ...
                <ScrollArea className="max-h-[300px]">
                  <div className="space-y-2">
                    {perceptionHistory.slice(1).map((p) => renderPerceptionCard(p))}
                  </div>
                </ScrollArea>
              </div>
            )}
